---
description: BoxLang AI Module 3.2.0 — Image Generation, Web Search, Fluent Audio Builders, Agent Registry, MCP Observability & more.
icon: star
---
...
BoxLang AI 3.2.0 introduces **image generation** (`aiImage()`), full **web search integration** (`webSearch()` and `webSearchAsync()`), a **fluent builder API** for all audio BIFs, the **Agent Registry** for centralized agent management, **MCP server pause/resume**, **MCP client stats & observability**, and major **MCP server analytics improvements**.
...
### 🔍 Web Search Tools — `webSearch()` and `webSearchAsync()`

BoxLang AI now includes a unified web search system with normalized results and provider abstraction.

| BIF | Description |
|---|---|
| `webSearch( query, options )` | Synchronous web search across configured providers |
| `webSearchAsync( query, options )` | Async web search returning a `BoxFuture` |
...
**Result format:** every provider returns the same fields:
`title`, `url`, `snippet`, `publishedDate`, `domain`, `score`, `thumbnail`, `language`.

```javascript
// Direct usage
results = webSearch( "latest BoxLang AI updates", { provider: "brave", maxResults: 8 } )
...
**Documentation**: [Web Search Tools](../../main-components/web-search/README.md)
