...
// External gateway modules (e.g. bx-ai-gateway-slack) register under their own name
gatewayRegistry().register( new SlackGateway(), "bx-ai-gateway-slack" )
slack = aiGateway( "slack" )
...
- **`approve_always`/`approve_session` grants never persisted for async gateways** (Slack/HTTP) — missing identity on `GatewayContext` and an incomplete resume-path decision handler.
