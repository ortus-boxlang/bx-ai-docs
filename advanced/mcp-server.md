...
### Class-Based MCP Servers (v3.2.0+)

For larger applications, you can encapsulate your entire MCP server in a dedicated class that extends `MCPServer`. This provides better organization, reusability, and testability.
...
Once your class is defined, register it with the AI service at application startup:
...
class {
    ...
    function onApplicationStart() {
        ...
        // Or using the BIF shorthand
        // aiService().putServer( "my-app", new mcp.MyAppServer() )
...
| All config in one place | Separation of concerns (tools, resources, prompts, security) |
| Harder to test in isolation | Easily unit-testable |
| Good for scripts and prototypes | Good for production applications |
| | Supports inheritance and composition |

**Multiple Server Classes:**

You can create multiple server classes for different purposes:

```javascript
// mcp/PublicServer.bx
class extends="MCPServer" {
    function init() {
        super.init( name: "public", description: "Public API" )
        registerPublicTools()
        return this
    }
}
...
class extends="MCPServer" {
    function init() {
        super.init( name: "admin", description: "Admin Tools" )
        registerAdminTools()
